...
### A1.
Let \(X\in\{0,1\}\) and \(Y\in\{0,1\}\) have the joint distribution below.
...
### B1.
Convert the count table into a joint probability table \(P(\text{Risk Level}, \text{Sex})\).

### B2.
Compute the marginals:
1. \(P(\text{Sex} = \text{Male})\) and \(P(\text{Sex} = \text{Female})\)  
2. \(P(\text{Risk Level} = \text{Low})\) and \(P(\text{Risk Level} = \text{High})\)

### B3.
Compute each conditional probability:
1. \(P(\text{High} \mid \text{Male})\)  
2. \(P(\text{Low} \mid \text{Male})\)  
3. \(P(\text{High} \mid \text{Female})\)  
4. \(P(\text{Male} \mid \text{High})\)  
5. \(P(\text{Female} \mid \text{Low})\)

### B4.
Interpret in a sentence (plain English):
1. \(P(\text{High} \mid \text{Male})\)  
2. \(P(\text{Male} \mid \text{High})\)

### B5.
Compute:
1. \(P(\text{High and Male})\)  
2. \(P(\text{High or Male})\)  
...
### D3.
Compute \(P(X_1+X_2 \text{ is even} \mid X_1=3)\).
...
Let \(Y\in\{0,1\}\) with \(P(Y=1)=0.30\) (so \(P(Y=0)=0.70\)).  
You are also told:
...
Suppose we have three binary random variables \(H\), \(S\), and \(P\), where **\(H\in\{R,D\}\)**.  
You are told:
...
**A4.**  
1. \(P(A\mid B)=\dfrac{0.2}{0.5}=0.4\)  
2. \(P(B\mid A)=\dfrac{0.2}{0.3}=0.666\ldots\)  
3. \(P(A\cup B)=0.3+0.5-0.2=0.6\)
...
**B5.**  
1. \(P(High\cap Male)=76/200=0.38\)  
2. \(P(High\cup Male)=0.48+0.78-0.38=0.88\)
...
**B7.**
1. Independence condition: for example, \(P(High\cap Male)=P(High)P(Male)\) (equivalently \(P(High\mid Male)=P(High)\)).
2. Test: \(P(High\cap Male)=0.38\) but \(P(High)P(Male)=0.48\times 0.78=0.3744\). Since these differ, the variables are **not independent** in this sample.
...
3. \(P(C\mid T^c)=4/132\approx 0.030\)
...
**D2.** Given \(X_1=6\), need \(X_2\ge 4\): \(3/6=0.5\)

**D3.** Given \(X_1=3\) (odd), sum is even iff \(X_2\) is odd: \(3/6=0.5\)

**D4.** If \(X_1=4\), “sum at least 9” means \(X_2\ge 5\).  
So \(P(A\mid B)=P(X_2\ge 5)=2/6=1/3\).
...
**E1.**  
1. \(P(A\mid B)=\dfrac{P(A,B)}{P(B)}\)  
2. \(P(A,B)=P(A\mid B)P(B)\)  
3. \(P(A,B)=P(B\mid A)P(A)\)

**E2.** \(P(A,B)=0.60\times 0.25=0.15\)

**E3.** \(P(A,B)=0.15\times 0.40=0.06\)
...
=\frac{0.80(0.30)}{0.38}=\frac{0.24}{0.38}\approx 0.632.
\]
...
**G1.**  
Because \(H\in\{R,D\}\), the event \((S=D, P=D)\) can occur only with \(H=R\) or \(H=D\). So:
